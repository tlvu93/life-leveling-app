  ...
  return (
    <View
      ...
      <View style={[styles.hud, { top: insets.top + 12 }]}>
        <Text testID="universe-tier" style={[styles.tier, { color: theme.ink, borderColor: theme.borderSoft, backgroundColor: theme.surfaceMuted }]}>
          {tierLabels[tier]}
        </Text>
        <Pressable testID="universe-zoom-in" accessibilityLabel="Zoom in" onPress={camera.zoomIn} style={[styles.control, { borderColor: theme.borderSoft, backgroundColor: theme.surfaceMuted }]}>
          <Text style={{ color: theme.ink }}>+</Text>
        </Pressable>
        <Pressable testID="universe-zoom-out" accessibilityLabel="Zoom out" onPress={camera.zoomOut} style={[styles.control, { borderColor: theme.borderSoft, backgroundColor: theme.surfaceMuted }]}>
          <Text style={{ color: theme.ink }}>-</Text>
        </Pressable>
        <Pressable testID="universe-fit" accessibilityLabel="Fit the whole Universe" onPress={camera.fitWorld} style={[styles.control, { borderColor: theme.borderSoft, backgroundColor: theme.surfaceMuted }]}>
          <Text style={[styles.controlLabel, { color: theme.ink }]}>Fit</Text>
        </Pressable>
        <Pressable testID="universe-back" accessibilityLabel="Back to Discover" onPress={() => router.push('/')} style={[styles.control, { borderColor: theme.borderSoft, backgroundColor: theme.surfaceMuted }]}>
          <Text style={[styles.controlLabel, { color: theme.ink }]}>Exit</Text>
        </Pressable>
      </View>

      {selected && (
        <View testID="universe-inspector" style={[styles.inspector, { borderColor: theme.borderSoft, backgroundColor: theme.surfaceMuted, bottom: insets.bottom + 16 }]}>
